...
alias cat='bat'
alias cd='z'
alias dl='aria2c -c -d /data/Downloads/ -x 8 -s 8 -k 1M --min-split-size=1M --file-allocation=trunc --max-tries=0 --retry-wait=5 --summary-interval=0 --console-log-level=warn'
...
upall() {
    ...
    if command -v needs-restarting >/dev/null && ! sudo needs-restarting -r; then
        echo "⚠️ Reinicialização recomendada."
        read -p "Reiniciar agora? [s/N]: " ans

        [[ "$ans" =~ ^[Ss]$ ]] && systemctl reboot
    fi
}
